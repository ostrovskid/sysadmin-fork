...
sources_checked:
  ...
  - https://applecensorship.com/app-store-monitor/app/6673731168
  - https://applecensorship.com/app-store-monitor/app/6746188973
  - https://applecensorship.com/app-store-monitor/app/6788279553
  - https://te-st.org/2026/08/02/vpndelete/
  - https://vc.ru/services/3001429-prilozhenie-happ-snova-v-rossiyskom-app-store
---

# Клиентские приложения: карта на 2026-05-24, точечно пересверена 11.08.2026
...
<!-- 2026-08-11: точечная ревизия — версии sing-box (1.13.18) и SFA (1.13.16),
     статус Happ в RU App Store (переизданный листинг), снятие sing-box-vt -->

> 🗓 **Что когда проверялось.** База карты — **24.05.2026**. Точечная ревизия
> **11.08.2026** пересверила только: версии ядра sing-box (мейнлайн 1.13.18) и
> SFA (1.13.16), статус Happ в RU App Store вместе с хронологией его листингов,
> снятие `sing-box-vt` со всех витрин App Store. Всё остальное — включая статус
> Hiddify и Karing в RU App Store — осталось майским и в этой ревизии заново не
> проверялось; такие места помечены датой наблюдения по месту.


> 🎯 **Основной рекомендованный клиент агента — `Happ`** («Happ — Proxy
> Utility», официальный сайт **happ.info**; старый адрес happ.su отдаёт на него
> 301-редирект, поэтому прежние ссылки в этом файле продолжают работать —
> наблюдение 2026-08-11, см. `vpn-consultation-flow.md` §2.2).
> Кроссплатформенный (iPhone, Android, Windows, macOS, Linux),
...
>
> **Про установку (пересверено 2026-08-11):** единого ответа «в RU App Store
> клиентов нет, меняй регион» больше нет — статус разный по клиентам и меняется
> волнами, поэтому его проверяют **в момент задачи**, а не берут из этого файла.
> **Happ** живёт циклом «снимают → переиздают»: по состоянию на 03.08.2026 в
> российской витрине был доступен переизданный листинг id6788279553 (хронология —
> §2.2), поэтому сперва ищем актуальный листинг Flyfrog LLC и только при его
> отсутствии меняем **регион App Store** (Казахстан/Армения и т.п.; для бесплатных
> приложений оплата не требуется). **По Hiddify свежих данных в ревизии
> 2026-08-11 не добывали:** последнее, что есть, — наблюдение оператора
> 2026-05-24 «в RU App Store нет», заново оно не проверялось, как проверенное
> сегодня его не подавать. На десктоп Happ ставится просто **с сайта happ.info**
> (старые ссылки вида https://www.happ.su/main/ru ведут туда же через редирект).
> Старый аргумент «рекомендуем
> Hiddify, потому что Happ удалён, а Hiddify — нет» **не работает**: удаление
> из RU App Store бьёт по обоим одинаково.
...
- **iOS** — `Happ` (основной) / `Hiddify` / `Karing`. Happ в RU App Store был
  доступен переизданным листингом (id6788279553, проверка 03.08.2026 — §2.2);
  по Hiddify свежих данных нет — последнее наблюдение 24.05.2026 «нет»
  → сперва искать актуальный листинг Happ, и только если его нет — смена региона
  App Store. Если какой-то клиент уже установлен — не удалять (переустановить
  без смены региона не получится).
...
### 1.1 Как ДОБАВИТЬ конфиг в клиент (грабля Камеры iOS)

`vless://`-ссылку заводят в клиент двумя способами:
- **Из буфера обмена (надёжнее всего):** скопировать ссылку → в Happ «+» → «Добавить из буфера».
- **QR внутри приложения:** в Happ «+» → «Сканировать QR» → навести на код.

⚠️ **НЕ сканировать QR штатной Камерой iPhone/Android.** Камера декодирует `vless://`, но не знает
эту схему и отвечает **«нет пригодных данных»** (наблюдалось у оператора 2026-07-15 — QR валиден,
тупик именно в Камере ОС). QR читает встроенный сканер клиента (Happ/Hiddify), которому знаком
`vless://`. Для новичка/семьи самый безотказный путь — «добавить из буфера», без QR вообще.
...
| 28 июня 2026 | Последний день, когда `sing-box-vt` фиксируется доступным в App Store (мониторинг Apple Censorship) |
| 27 июля 2026 | `sing-box-vt` недоступен во **всех 75** проверенных сторфронтах App Store; карточка приложения отдаёт 404 |
...
### 2.2 Состояние альтернатив на iOS (база 2026-05-15; Happ и sing-box-vt пересверены 11.08.2026)

| Клиент | App Store ID | Ядро | Версия ядра | RU App Store |
|---|---|---|---|---|
| **Happ** | 6504287215 (исходный) + переиздания | Xray-core (по умолч.) + опция sing-box | iOS 5.4.0 → **26.7.28**; Android 4.1.0 → **26.6.27** (замер 12.08.2026, см. врезку ниже) | Исходный листинг снят в волне ~28.03.2026, дальше цикл «снимают → переиздают» (хронология под таблицей). На 03.08.2026 в RU App Store был **доступен** переизданный листинг id6788279553 |
...
| **sing-box-vt** | 6673731168 | sing-box 1.11.x | 1.11.4 (24.02.2025) | **Снят со всех витрин App Store.** Последний раз доступен 28.06.2026, на 27.07.2026 недоступен во всех 75 проверенных сторфронтах, карточка отдаёт 404. К мартовской волне отношения не имеет — снятие произошло позже. Новая установка невозможна, смена региона не помогает |
...
> ⚠️ **«Трекает свежий Xray» — не значит «одинаково свежий на всех платформах».**
> Замер на живых устройствах 12.08.2026 (наблюдение оператора, обе версии видны в самом
> приложении): iOS, Happ `5.4.0` (API 3.19.0) — ядро Xray **26.7.28**; Android 13,
> Happ `4.1.0(1899)` — ядро **26.6.27**. Ветки нумеруются независимо (iOS 5.x против
> Android 4.x), и Android на момент замера отставал на релиз ядра.
>
> **Почему это важно, а не мелочь.** Именно версия ядра ВНУТРИ клиента решает, пройдёт
> ли он порог `minClientVer` на сервере: с Xray-core `v26.7.11` REALITY-сервер с пустым
> полем отбивает всех, чьё ядро старее `26.3.27`, — молча, без ошибки в логе сервера
> (`transports.md` §13.5, рефлекс персоны §3.8.13). Обобщение «Happ всегда свежий»
> заставляет пропустить эту проверку. **MUST** смотреть версию ядра в самом приложении
> на конкретном устройстве, а не выводить её из номера версии приложения или из того,
> что «клиент обычно свежий».
>
> Замер — одна точка на платформу, а не свойство всех сборок: у другого пользователя
> может стоять более старая версия приложения с более старым ядром. Перепроверять при
> задачах, где порог версии имеет значение.

**Хронология листингов Happ в App Store (проверено 2026-08-11).** Исходный листинг
id6504287215 снят в мартовской волне 2026. Переиздание «Happ - Proxy Utility Plus»
(id6746188973, разработчик Flyfrog LLC) последний раз фиксировалось доступным
22.06.2026; **24.06.2026 снято по требованию РКН** (подтверждено te-st.org,
zona.media, ixbt, 4pda, anti-malware.ru); **29.06.2026 вернулось** в магазин под
названием «Happ — Proxy Utility+» — на тот день оно открывалось по прямой ссылке,
но в поиске не индексировалось (дословно vc.ru, 29.06.2026: «Сервис пока не
проиндексирован в магазине, поэтому найти его через поиск нельзя»); индексация
после этой даты отдельно не перепроверялась. Листинг «Happ - Proxy Utility Plus»
id6788279553 датирован 19.07.2026 и **по состоянию на 03.08.2026** был доступен
**только** в российской витрине (проверен в 3 сторфронтах — доступен в 1, РФ).
Каждая строка выше — состояние на свою дату наблюдения, а не сегодняшнее состояние
магазина. Был ли между 29.06 и 19.07.2026 ещё один промежуточный листинг — не
установлено. Практический вывод: **до совета «меняй регион» проверять текущий
листинг Flyfrog LLC**, переизданный Happ в RU App Store может быть доступен.

**Про sing-box-vt (проверено 2026-08-11).** Офиц. документация sing-box:
обновления приложений в App Store заблокированы ревью Apple, бета доступна только
через TestFlight и только спонсорам. То есть путь «поставить свежий sing-box-vt»
на iOS закрыт полностью, а не только в российской витрине.

> ⚠️ **Ключевой вывод по RU App Store (пересверено 2026-08-11).** Общего «в
> российском App Store нет ни Happ, ни Hiddify, поэтому для любого клиента нужна
> смена региона» больше нет: статус разный по клиентам и фиксируется **на дату
> наблюдения**, а не как постоянное свойство магазина.
>
> - **Happ** — по состоянию на 03.08.2026 в российской витрине был доступен
>   переизданный листинг id6788279553 (хронология выше). Смена региона может и не
>   понадобиться: сперва ищем актуальный листинг Flyfrog LLC, и проверяем его в
>   момент задачи (сайт happ.su жив — HTTP 200, наблюдение 2026-05-24).
> - **Hiddify** — свежих данных в ревизии 2026-08-11 **не добывали**. Последнее,
>   что есть: наблюдение оператора 2026-05-24 «в RU App Store нет». Как
>   проверенный сегодня статус оно не годится — уточнять отдельно.
>
> «Не удалён» само по себе больше **не аргумент** в пользу какого-либо клиента.
> Колонка выше фиксирует и историческую волну удалений (~28.03.2026), и статус на
> дату проверки — это разные вещи: волна была, и она затронула обоих кандидатов.
> Статус меняется волнами — при сомнении проверять напрямую /
> `/refresh-vpn-knowledge LAYER=live`.

**Рекомендация для iOS:**
- **Новое устройство в РФ** — `Happ` (основной; happ.su): сперва искать актуальный
  переизданный листинг в RU App Store (на 03.08.2026 был доступен id6788279553), и
  только при его отсутствии — смена региона. Альтернативы — `Hiddify Proxy & VPN`,
  `Karing`: их статус в RU App Store с 24.05.2026 не пересверялся (по Hiddify тогда
  «нет», по Karing — не установлено), поэтому сперва проверить листинг, и уже при
  его отсутствии — смена региона.
- **Старое устройство, уже что-то стоит** — не удалять. Любой работающий клиент
  ...
  региона не получится, а `sing-box-vt` — не получится вообще: он снят со всех
  витрин App Store.
...
| Клиент | Версия ядра sing-box внутри | Отставание от мейнлайна (1.13.18) |
|---|---|---|
...
| F-Droid version | 1.13.16 (build 708, 5 августа 2026, 92 MiB) |
| Min Android | 6.0+ |
...
| App Store (iOS/macOS) | id6504287215 (исходный) — снят в волне ~28.03.2026; в RU App Store живёт переизданными листингами, на 03.08.2026 доступен id6788279553 (хронология — §2.2). Смена региона нужна, только если актуального листинга нет |
...
`xray-mac-chain.md`). Xray это умеет, sing-box detour на outbound-протоколах —
нет: issue **SagerNet/sing-box#3205** «Detour doesnt work on outbound protocols»
закрыт как **not planned**.

> ⚠️ **Оговорка по источнику.** В самом #3205 сообщается о падении связки
> **VLESS→Trojan**. Вывод «sing-box chain VLESS→VLESS не работает» — наша
> экстраполяция от общего «detour на outbound-протоколах не планируется»,
> отдельным источником он не подтверждён; при необходимости проверять практикой.
> Прежняя ссылка на #1562 была битой (это issue про отвал WireGuard, к detour
> отношения не имеет) — не возвращать её.
...
- Версия — мейнлайн **v1.13.18** (09.08.2026), идентично Linux. Ветка 1.14 на
  11.08.2026 всё ещё в бете (v1.14.0-beta.14), стабильного релиза нет.
...
|---|---|---|
| `sing-box-vt` (он же SFM) | **Из App Store снят** (27.07.2026, §2.2), тот же ID что iOS, macOS 13.0+; работает только там, где уже установлен | Версия 1.11.4 (отстаёт) |
...
- Версия — мейнлайн v1.13.18 (09.08.2026).
...
|---|---|---|
| sing-box mainline CLI (Linux/Win/macOS) | 1.13.18 (09.08.2026) | Latest |
| SFA (Android, F-Droid) | 1.13.16 (05.08.2026) | Latest |
